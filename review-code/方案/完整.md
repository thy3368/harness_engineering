...
| 阶段 | 周期 | 内容 | 交付物 |
|---|---|---|---|
| **P0 基线搭建** | 第 1-2 周 | 统一 clang-format / clang-tidy 配置；CI 接入编译 + 单测 + 覆盖率 | `.clang-format` `.clang-tidy` CI 流水线 |
| **P1 静态分析** | 第 3-4 周 | 接入 cppcheck + Clang SA；清理存量告警；设定 error 级零容忍 | 静态分析报告、告警清零基线 |
| **P2 质量平台** | 第 5-6 周 | 部署 SonarQube；接入 OSV-Scanner；统一 SARIF 汇聚 | 质量门禁看板、依赖安全报告 |
| **P3 契约兼容** | 第 7-8 周 | 接入 abidiff + buf breaking；建立基线版本管理；Pact 契约试点 | ABI 基线、协议兼容性检查 |
| **P4 效能运营** | 持续 | 阈值调优、增量分析提速、本地 pre-commit 全覆盖、豁免流程规范化 | 效能度量报告、门禁 SLA |
